...
impl OAuth2Error {
    ...
    pub fn from_basic_error(
        error: RequestTokenError<Error<ReqwestError>, BasicErrorResponse>,
    ) -> OAuth2Error {
        match error {
            RequestTokenError::ServerResponse(e) => {
                ...
                OAuth2Error::new(
                    error.to_string(),
                    e.error_description().map(|s| s.to_string()),
                )
            }
    ...
    pub fn from_device_error(
        error: RequestTokenError<Error<ReqwestError>, DeviceCodeErrorResponse>,
    ) -> OAuth2Error {
        match error {
            RequestTokenError::ServerResponse(e) => {
                let error = e.error();
    ...
    pub fn from_configuration_error(error: ConfigurationError) -> OAuth2Error {
        match error {
            ...
            _ => OAuth2Error::new(
                error.to_string(),
                Option::from(error).map(|s| s.to_string()),
            ),
        }
    }
}
